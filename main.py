...
class App(tk.Tk):
    def __init__(self):
        ...
        # Delivery state — 智联
        self.zhilian_page = None
        self.zhilian_logged_in = False
        self.zhilian_delivery = None
    ...
    def on_close(self):
        self.delivery_running = False
        for d in [self.delivery, self.zhilian_delivery]:
            if d:
                d.running = False
        self.destroy()
...
class MainFrame(tk.Frame):
    ...
    def _build_delivery_tab(self, parent):
        ...
        # 今日总投递数
        self.today_total_label = tk.Label(stats_title_row, text='今日总投递: 0  (BOSS: 0  智联: 0)',
                                          font=('Microsoft YaHei UI', 10, 'bold'),
        ...
        self.boss_count_label = tk.Label(stats_detail_row, text='BOSS: 0', font=('Microsoft YaHei UI', 10, 'bold'),
                                          fg=c['accent'], bg=c['bg2'])
        self.boss_count_label.pack(side=tk.LEFT, padx=(12, 16))

        self.zhilian_count_label = tk.Label(stats_detail_row, text='智联: 0', font=('Microsoft YaHei UI', 10, 'bold'),
                                            fg=c['green'], bg=c['bg2'])
        self.zhilian_count_label.pack(side=tk.LEFT)
        ...
        # 平台勾选
        self.boss_platform_var = tk.BooleanVar(value=True)
        tk.Checkbutton(login_header, text='BOSS直聘', variable=self.boss_platform_var,
                       font=('Microsoft YaHei UI', 9), bg=c['bg2'], fg=c['accent'],
                       selectcolor=c['bg3'], activebackground=c['bg2']).pack(side=tk.LEFT, padx=(16, 8))

        self.zhilian_platform_var = tk.BooleanVar(value=True)
        tk.Checkbutton(login_header, text='智联招聘', variable=self.zhilian_platform_var,
                       font=('Microsoft YaHei UI', 9), bg=c['bg2'], fg=c['green'],
        ...
        # 智联登录行
        zhilian_row = tk.Frame(login_inner, bg=c['bg2'])
        zhilian_row.pack(fill=tk.X)

        zhilian_tag = tk.Frame(zhilian_row, bg=c['green'], padx=0, pady=0, width=4, height=20)
        zhilian_tag.pack(side=tk.LEFT, padx=(0, 8))
        zhilian_tag.pack_propagate(False)

        tk.Label(zhilian_row, text='智联招聘', font=('Microsoft YaHei UI', 9, 'bold'),
                 fg=c['green'], bg=c['bg2'], width=8, anchor='w').pack(side=tk.LEFT)

        self.zhilian_login_btn = ttk.Button(zhilian_row, text='打开浏览器登录', style='Accent.TButton',
                                             command=self.get_zhilian_qr_code)
        self.zhilian_login_btn.pack(side=tk.LEFT)

        ttk.Button(zhilian_row, text='清空', style='Ghost.TButton',
                   command=lambda: self._clear_browser_cache('zhilian')).pack(side=tk.LEFT, padx=(4, 0))

        self.zhilian_login_status_label = tk.Label(zhilian_row, text='未登录', font=('Microsoft YaHei UI', 9, 'bold'),
                                                    fg=c['red'], bg=c['bg2'])
        self.zhilian_login_status_label.pack(side=tk.LEFT, padx=12)
        ...
        # 智联日志卡片
        zhilian_log_card = tk.Frame(right, bg=c['bg2'], highlightthickness=1, highlightbackground=c['bg4'])
        zhilian_log_card.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        # 彩色顶条
        tk.Frame(zhilian_log_card, bg=c['green'], height=2).pack(fill=tk.X)

        zhilian_log_inner = tk.Frame(zhilian_log_card, bg=c['bg2'], padx=14, pady=10)
        zhilian_log_inner.pack(fill=tk.BOTH, expand=True)

        zhilian_log_header = tk.Frame(zhilian_log_inner, bg=c['bg2'])
        zhilian_log_header.pack(fill=tk.X, pady=(0, 4))

        tk.Label(zhilian_log_header, text='● 智联招聘 运行日志', font=('Microsoft YaHei UI', 10, 'bold'),
                 fg=c['green'], bg=c['bg2']).pack(side=tk.LEFT)
        ttk.Button(zhilian_log_header, text='清空', style='Ghost.TButton',
                   command=lambda: self._clear_platform_log('zhilian')).pack(side=tk.RIGHT)

        self.zhilian_log_text = scrolledtext.ScrolledText(zhilian_log_inner, font=('Consolas', 10), bg='#0a0c14',
                                                           fg=c['text2'], insertbackground=c['text'],
                                                           relief='flat', bd=0, highlightthickness=1,
                                                           highlightbackground=c['bg4'])
        self.zhilian_log_text.pack(fill=tk.BOTH, expand=True)

        # 通用日志（保留兼容）
        self.log_text = self.boss_log_text

        # Tag colors for both
        for log_widget in [self.boss_log_text, self.zhilian_log_text]:
    ...
    def on_show(self):
        ...
        if self.app.login_mode == 'self_ai':
            ...
            if self.app.logged_in or self.app.zhilian_logged_in:
                self.start_btn.state(['!disabled'])
            else:
                self.start_btn.state(['disabled'])
    ...
    def _clear_platform_log(self, platform):
        widget = self.boss_log_text if platform == 'boss' else self.zhilian_log_text
        widget.delete(1.0, tk.END)
        widget.insert(tk.END, '日志已清空\n')

    def _clear_log(self):
        for w in [self.boss_log_text, self.zhilian_log_text]:
            w.delete(1.0, tk.END)
            w.insert(tk.END, '日志已清空\n')

    def _clear_browser_cache(self, platform):
        """清空浏览器缓存和登录状态，以便重新登录"""
        import shutil
        if not messagebox.askyesno('确认', f'确定要清空{platform}浏览器缓存和登录状态吗？\n\n这将删除浏览器数据（包括已保存的密码等）。'):
            return
        ...
        if platform == 'boss':
            ...
            self._update_login_ui('boss', False)
            self.boss_login_status_label.config(text='未登录', fg=self.app.c['red'])
        else:
            cookie_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookies_zhilian.json")
            user_data = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'browser_data_zhilian')
            if self.app.zhilian_page:
                try:
                    self.app.zhilian_page.quit()
                except:
                    pass
                self.app.zhilian_page = None
            self.app.zhilian_logged_in = False
            self._update_login_ui('zhilian', False)
            self.zhilian_login_status_label.config(text='未登录', fg=self.app.c['red'])
        ...
        self.log_msg(f'{platform} 缓存已清空，可重新登录')
        # 重置启动按钮
        self.start_btn.state(['disabled'])
    ...
    def _update_saved_login_status(self):
        ...
        if os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookies_zhilian.json")):
            self.zhilian_login_status_label.config(text='已保存登录(待验证)', fg=self.app.c['orange'])
    ...
    def _init_browser(self, platform='boss'):
        """初始化浏览器，platform: 'boss' 或 'zhilian'"""
        try:
    ...
    def _verify_saved_login(self):
        def _run():
            try:
                if not self.app.page and not self._init_browser('boss'):
                    return
                # 如果浏览器被隐藏，先显示
                try:
                ...
                if 'user' not in url and 'login' not in url:
                    ...
                    self._save_platform_cookies('boss')
                    self.after(0, self._update_login_ui, 'boss', True)
    ...
    def _poll_boss_login(self):
        ...
        try:
            ...
            if 'zhipin.com/web/geek' in url or ('zhipin.com' in url and 'user' not in url and 'login' not in url):
                self.app.logged_in = True
                self._save_platform_cookies('boss')
                self._update_login_ui('boss', True)
    ...
    def get_zhilian_qr_code(self):
        self.zhilian_login_btn.state(['disabled'])
        self.zhilian_login_btn.config(text='初始化中...')
        self.zhilian_login_status_label.config(text='初始化中...', fg=self.app.c['orange'])

        def _run():
            try:
                if not self.app.zhilian_page and not self._init_browser('zhilian'):
                    self.after(0, self.zhilian_login_btn.config, {'state': 'normal', 'text': '打开浏览器登录'})
                    return
                # 如果浏览器已存在但被隐藏，先显示
                try:
                    self.app.zhilian_page.set.window.show()
                except:
                    pass
                self.app.zhilian_page.get('https://www.zhaopin.com/')
                time.sleep(2)
                self.log_msg('请在智联浏览器窗口中登录（扫码或账号密码）')
                self.after(0, self.zhilian_login_status_label.config,
                           {'text': '请登录智联', 'fg': '#1890ff'})
                self.after(0, self.zhilian_login_btn.config, {'state': 'normal', 'text': '打开浏览器登录'})
                self._poll_zhilian_login()
            except Exception as e:
                self.log_msg(f'获取智联登录页失败: {e}')
                self.after(0, self.zhilian_login_status_label.config, {'text': '获取失败', 'fg': self.app.c['red']})
                self.after(0, self.zhilian_login_btn.config, {'state': 'normal', 'text': '打开浏览器登录'})

        threading.Thread(target=_run, daemon=True).start()

    def _poll_zhilian_login(self):
        if not self.app.zhilian_page:
            return
        try:
            url = self.app.zhilian_page.url.lower()
            page = self.app.zhilian_page

            # 检测1: URL 跳转到个人中心（登录成功标志）
            if 'i.zhaopin.com' in url:
                self._set_zhilian_logged_in()
                return

            # 检测2: 页面中是否有用户相关元素（已登录标志）
            try:
                user_indicators = [
                    '.user-avatar', '.user-name', '.user-center',
                    '.header__user', '.mycenter', '.personal-center',
                    '.resume-name', '[class*="user"]', '.login-user',
                ]
                for sel in user_indicators:
                    try:
                        elem = page.ele(sel, timeout=0.5)
                        if elem:
                            self._set_zhilian_logged_in()
                            return
                    except:
            ...
            # 检测3: URL 不是登录/注册页且有 zhaopin cookie
            if 'zhaopin.com' in url and 'passport' not in url and 'login' not in url:
                try:
                    cookies = page.cookies()
                    for c in cookies:
                        # 智联登录后常见 cookie
                        if c.get('name', '') in ('at', 'rt', 'zhaopin_sid', 'login_name', 'user_id'):
                            self._set_zhilian_logged_in()
                            return
                except:
                    pass
            ...
            pass
        self.after(2000, self._poll_zhilian_login)

    def _set_zhilian_logged_in(self):
        self.app.zhilian_logged_in = True
        self._save_platform_cookies('zhilian')
        self._update_login_ui('zhilian', True)
        self.log_msg('智联招聘登录成功! 浏览器已隐藏(后台运行)')
        # 登录成功后隐藏浏览器窗口
        try:
            self.app.zhilian_page.set.window.hide()
        except:
            try:
                self.app.zhilian_page.set.window.mini()
            except:
                pass

    def _save_platform_cookies(self, platform):
        try:
            page = self.app.page if platform == 'boss' else self.app.zhilian_page
            if not page:
                return
            cookies = page.cookies()
            if platform == 'boss':
    ...
    def _update_login_ui(self, platform, logged_in):
        c = self.app.c
        if platform == 'boss':
            if logged_in:
            ...
            if logged_in:
                self.zhilian_login_status_label.config(text='已连接', fg=c['green'])
                self.zhilian_login_btn.config(text='重新登录')
            else:
                self.zhilian_login_status_label.config(text='未登录', fg=c['red'])

        # 任一平台登录且许可证有效即可启用开始按钮
        if (self.app.logged_in or self.app.zhilian_logged_in) and \
    ...
    def start_search_thread(self):
        ...
        # 检查选中平台的登录状态
        selected = []
        if self.boss_platform_var.get():
            if not self.app.logged_in:
                messagebox.showwarning('提示', 'BOSS直聘未登录，请先登录或取消勾选!')
                return
            selected.append('boss')
        if self.zhilian_platform_var.get():
            if not self.app.zhilian_logged_in:
                messagebox.showwarning('提示', '智联招聘未登录，请先登录或取消勾选!')
                return
            selected.append('zhilian')

        if not selected:
            messagebox.showwarning('提示', '请至少选择一个已登录的平台!')
            return
        ...
        # 投递开始：隐藏所有浏览器
        for pg in [self.app.page, self.app.zhilian_page]:
            if pg:
                try:
                    pg.set.window.hide()
                except:
        ...
        mode = self.mode_var.get()
        platform_names = {'boss': 'BOSS', 'zhilian': '智联'}
        names = [platform_names[p] for p in selected]
        self.status_text.config(text=f'投递中 ({"/".join(names)})...')
    ...
    def _search_thread(self, platform, mode):
        try:
            ...
            if platform == 'boss':
                ...
                from zhilian_delivery import ZhilianDeliveryDP
                page = self.app.zhilian_page
                delivery = ZhilianDeliveryDP(page, self.app.analyzer)
                self.app.zhilian_delivery = delivery
            ...
            if platform == 'boss':
                ...
                from config import ZHI_LIAN_CITY_MAP
                city_code = next((code for code, name in ZHI_LIAN_CITY_MAP.items() if name == city_name), '530')
            ...
            if mode == 'auto':
                ...
                if platform == 'boss':
                    ...
                    self.app.zhilian_page = delivery.page
                # 智联投递完成后，只保留第一个页签
                if platform == 'zhilian':
                    self._cleanup_zhilian_tabs(delivery.page)
                self.after(0, lambda p=platform: self._on_platform_complete(p, True))
            else:
                if platform == 'boss':
                    ...
                    delivery.search_and_deliver(
                        city_code=city_code, city_name=city_name,
                        title_keywords=title_keywords, threshold=0,
                        resume_text=self.app.resume_text, mode='manual',
                        callback=self._collect_callback
                    )
                if platform == 'boss':
                    self.app.page = delivery.page
                ...
                if platform == 'zhilian':
                    self._cleanup_zhilian_tabs(delivery.page)
                self.after(0, lambda p=platform: self._on_platform_complete(p, False))
        except Exception as e:
            self.log_msg(f'[{platform}] 搜索异常: {e}')
            import traceback
            traceback.print_exc()
            self.after(0, lambda p=platform: self._on_platform_complete(p, True))
    ...
    def _update_delivery_stats(self, platform, session_count):
        ...
        if platform == 'boss':
            ...
            self.zhilian_session_count = session_count
            self.zhilian_count_label.config(text=f'智联本次: {session_count}')

        # 获取今日数据库统计
        if self.app.user_id:
            stats = get_today_delivery_stats(self.app.user_id)
            self.today_total_label.config(
                text=f'今日总投递: {stats["total"]}  (BOSS: {stats["boss"]}  智联: {stats["zhilian"]})'
            )
    ...
    def _on_platform_complete(self, platform, is_auto):
        ...
        # 智联完成后清理多余页签
        if platform == 'zhilian':
            self._cleanup_zhilian_tabs(self.app.zhilian_page)

        if boss_done and zhilian_done:
            ...
            zhilian_cnt = self.app.zhilian_delivery.delivery_count if self.app.zhilian_delivery else 0
            total = boss_cnt + zhilian_cnt

            if is_auto:
                boss_session = self.boss_session_count
                zhilian_session = self.zhilian_session_count
                total_session = boss_session + zhilian_session
                ...
                self.log_msg(f'全部投递完成! 本次:BOSS:{boss_session} + 智联:{zhilian_session} = {total_session} | 今日总:{stats["total"]}')
            else:
    ...
    def stop_delivery(self):
        ...
        if self.app.zhilian_delivery:
            self.app.zhilian_delivery.running = False
        self.log_msg('已停止所有平台投递')
        self.stop_btn.state(['disabled'])
        self.start_btn.state(['!disabled'])
    ...
    def do_logout(self):
        self.app.delivery_running = False
        for d in [self.app.delivery, self.app.zhilian_delivery]:
            if d:
                d.running = False
